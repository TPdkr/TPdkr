# COMPUTER SCIENCE BACHELOR'S STUDENT
## About me:
***3nd year computer Science student at constructor university bremen***. I like to code and solve challenging problems.
I am currently also making a website as a cv/profile. One of my repos is a lanuage learning app you can download.

I also have a personal website that has links and descriptions of most of my projects and experiences.
[personal website main page](https://tpdkr.github.io/cv_page2/)

## Skills
***Languages:***
- Kotlin, C, C++, Python, PHP, JavaScript, Haskell

***Web technologies:***
- HTML, CSS, React, SQL

***Developer tools:***
- Git, GitHub, Linux, Makefiles, LateX

***IDEs:***
- VS Code, IntelliJ IDEA, Android Studio

# REPOSITORIES
Here are the main groups of my repositories listed below. Most of them fall into one of the following.

## CV website 2.0 and 1.0
#### 2.0 (2026)
This is a reworked 1.0 with React intead of plain HTML,JS also with more content and better style.
- [repository page](https://github.com/TPdkr/cv_page2)
- [website main page](https://tpdkr.github.io/cv_page2/)

#### 1.0 (2025)
Never truly finished as the projects page is empty
- [repository page](https://github.com/TPdkr/cv_page)
- [website main page](https://tpdkr.github.io/cv_page/)

## Computer vision course (2025)
These are my computer vision assignment with Python grouped into a single repo. Published after completion of the course.
- [repository page](https://github.com/TPdkr/computer_vision_cub_hw)

## Software engineering repo (2025)
- [repository page](https://github.com/AnuraagDeshpande/SoftwareSuperProject_repo)

## Genau(Learner) German vocabulary app (2025)
An android app for learning German vocabulry in a nice an simple way. Organize words and courses,
get motivated and have fun.
- [repository page](https://github.com/TPdkr/learner)

## Causal Machine laerning hackathon(2024)
A hackathon where I participated during Fall 2024.

## OS (C/Linux)(2024)
I have uploaded coding assignments for the Operating systems course, which I took in Fall 2024.
They are on topics such as pipes, forking, signals, semaphores and Internet protocol.

- [hw 10](https://github.com/TPdkr/OS-hw10-communication-in-C-with-sockets)
- [hw 9](https://github.com/TPdkr/OS-hw9-pipes-and-exec-in-C)
- [hw 8](https://github.com/TPdkr/OS-hw8-paging-and-memory-mapping)
- hw 6-7 had no coding parts
- [hw 5](https://github.com/TPdkr/OS-hw5-POSIX-cond-var-and-mutexes)
- [hw 4](https://github.com/TPdkr/OS-hw4-multithreading-and-exclusive-locks)
- [hw 3](https://github.com/TPdkr/OS-hw-3-threads-in-C)
- [hw 2](https://github.com/TPdkr/OS-homework-2)
- [hw 1](https://github.com/TPdkr/OS-homework-1)

## Kotlin course assignments(2024)
Assignments of the Kotlin course I took in Spring 2024. They tackle problems such as
asynchronous and object oriented programming. A lot of assignments were 
different courses to complete in a limited amount of time.

- [Coroutines and channels](https://github.com/TPdkr/Coroutines_and_channels)
- [Hanoi towers](https://github.com/TPdkr/Hanoi_Towers)
- [CSC kotlin course part 3](https://github.com/TPdkr/CSC_Kotlin_Course_Part3)
- [Refractoring course](https://github.com/TPdkr/Refactoring_Course)
- [CSC kotlin course](https://github.com/TPdkr/CSC_Kotlin_Course)



<!--
**TPdkr/TPdkr** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
